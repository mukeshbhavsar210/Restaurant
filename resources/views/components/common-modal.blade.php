<div class="modal fade {{ $modal['formConfig']['modal'] }}" id="{{ $modal['modal_id'] }}" tabindex="-1">
    <div class="modal-dialog {{ $modal['formConfig']['modalSize'] }}" {{ $modal['formConfig']['modal_size'] ?? '' }}">
        <div class="modal-content">
            ...
            <form id="commonForm" action="{{ $modal['formConfig']['action'] }}" method="{{ $modal['formConfig']['method'] == 'GET' ? 'GET' : 'POST' }}" enctype="multipart/form-data">
                @csrf                
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" id="submitBtn">{{ $modal['formConfig']['button'] }}</button>
                </div>
            </form>
        </div>
    </div>
